"""
Slice 1: assign each project to exactly one (timeslot, room).
No panel assignment; lecturer availability not yet enforced.

OR-Tools CP-SAT API used:
  ...
  only_enforce_if()    — conditional constraint, active only when a
                         BoolVar is 1 (true)
...
def solve_slice1(cfg: Config) -> SolveResult:
    ...
    # Soft objective: compact schedule (minimise last used slot index)
    # only_enforce_if makes the lower-bound constraint conditional on x[p,t,r]
