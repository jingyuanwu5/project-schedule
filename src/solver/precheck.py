"""
Pre-solve checks that can catch trivially infeasible configs before
the solver ever runs. Returning plain-English messages is important
because the module coordinator is not expected to understand solver output.
"""
...
def precheck(cfg: Config) -> Tuple[List[str], List[str]]:
    ...
    if capacity < len(cfg.projects):
        errors.append(
            f"Not enough capacity: {cfg.constraints.rooms} room(s) × "
    ...
    for l in cfg.lecturers:
        ...
        if bad:
            errors.append(
                f"Lecturer '{l.id}' lists unknown slot id(s): {bad}"
            )
    ...
    for s in cfg.students:
        ...
        if bad:
            errors.append(
                f"Student '{s.id}' lists unknown slot id(s): {bad}"
            )
